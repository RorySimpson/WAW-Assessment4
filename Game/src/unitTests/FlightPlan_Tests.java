...
public class FlightPlan_Tests {
	...
	public void generateEntryPointTest1() {
		// Testing that the function generates a entrypoint that is
		// contained within the entrypoint list
		EntryPoint result = flight1.getFlightPlan()
				.generateEntryPoint(airspace);
		assertTrue(result == airspace.getListOfEntryPoints().get(0)
				|| result == airspace.getListOfEntryPoints().get(1)
				|| result == airspace.getListOfEntryPoints().get(2));

	}

	@Test
	public void generateVelocityTest() {
		// Testing that the velocity generated is always within the
		// constraints applied.
		for (int i = 0; i < 100; i++) {
	...
	public void buildRouteTest1() {
		...
		for (int i = 0; i < 100; i++) {
			ArrayList<Point> route = flightplan.buildRoute(airspace, flight1
	...
	public void buildRouteTest3() {
		...
		// Testing that it doesn't build a route if the airspace has no
		// waypoints
		...
		ArrayList<Point> route = flightplan.buildRoute(
				airspaceMissingExitPoints, flight1.getFlightPlan()
						.getEntryPoint());
	...
	public void buildRouteTest4() {
		...
		ArrayList<Point> route = flightplan.buildRoute(
				airspaceMissingWaypoints, flight1.getFlightPlan()
						.getEntryPoint());
		assertTrue(route.size() == 0);
	...
	public void updateFlightPlanTest1() {
		...
		// Tests that a waypoint is removed from the flightplan when
		// visited.
		...
		flight1.getFlightPlan().updateFlightPlan(new ScoreTracking());
		assertEquals(previousSize - 1, flight1.getFlightPlan()
	...
	public void updateFlightPlanTest2() {
		...
		// Tests that a waypoint is not removed from the flightplan when
		// not visited.
		int previousSize = flight1.getFlightPlan().getCurrentRoute().size();
		...
		flight1.getFlightPlan().updateFlightPlan(new ScoreTracking()); // {!}
																		...
																		// these
																		// properly
		assertEquals(previousSize, flight1.getFlightPlan().getCurrentRoute()
				.size(), 0);
