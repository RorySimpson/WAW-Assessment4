...
public class PlayState extends BasicGameState {
	/* All the images */
	protected static Image easyButton, mediumButton, hardButton, easyHover,
			mediumHover, hardHover, backgroundImage, difficultyBackground,
			statusBarImage, clockImage, windImage, flightIcon, messageBoxImage,
			scoreCoinImage, cursorImg, achievementBox, soundOffImage,
			soundOnImage, pauseImage, moonImage, sunImage, dayTimeImage;
	...
	/*
	 * A game over delay to allow the explosion to appear before displaying the
	 * game over screen
	 */
	...
	/*
	 * Overriding the enter state method so Eclipse doesn't prompt us with
	 * warnings
	 */
	@Override
	...
	public void init(GameContainer gc, StateBasedGame sbg)
...

		{
			...
			loading.add(new DeferredFile("res/graphics/new/moon.png") {
				public void loadFile(String filename) throws SlickException {
					moonImage = new Image(filename);
				}
			});

			loading.add(new DeferredFile("res/graphics/new/sun.png") {
				public void loadFile(String filename) throws SlickException {
			...
			SpriteSheet sheet = new SpriteSheet("res/graphics/explosion.png",
					128, 128); // Size
		...
		airspace.getListOfEntryPoints().add(
		...
		airspace.getListOfExitPoints().add(
	...
	public void drawEventMessage(GameContainer gc, Graphics g) {
		...
		boolean waitingToTakeOffAtAPL = false, waitingToTakeOffAtAPR = false, eruptionAboutToOccur = false, waitingToLand = false;

		/*
		 * Loop through flighs to check if any needs to land or take off and
		 * signal to display the message
		 */
		for (Flight flight : airspace.getListOfFlights()) {

			/*
			 * Check if there's a plane waiting to take off at the left airport
			 */
			...
			/*
			 * Check if there's a plane waiting to take off at the right airport
			 */
			if (flight.getFlightPlan().getEntryPoint() == airspace
					.getAirportRight().getEndOfRunway()
					&& !flight.isTakingOff() && flight.getAltitude() == 0) {
			 ...
			 */
			if ((flight.getFlightPlan().getCurrentRoute().get(0) == airspace
					.getAirportRight().getBeginningOfRunway() || flight
					.getFlightPlan().getCurrentRoute().get(0) == airspace
		...
		if (airspace.getEventController().getVolcano()
				.getCountdownTillNextEruption() < 600) {
	...
	public void render(GameContainer gc, StateBasedGame sbg, Graphics g)
		...
		if (settingDifficulty) {
			...
			/*
			 * Check if the user is hovering the buttons in order to render
			 * different images
			 */
			...
			/*
			 * Check if it's night or day. 25 is the minimum value that we drop
			 * to
			 */
			else {
				/*
				 * If it's complete night, start lighting up we only check for
				 * red because all the other colours are kept equal to red
				 */
				...
				if (red == MINIMUMBRIGHTNESS || red == MAXIMUMBRIGHTNESS) {
					/*
					 * If maximum or minimum brightness, don't change brightness
					 * for 20 seconds. This is done in order to have some delay
					 * between transitions
					 */
			...
			dayTimeImage = (red > (MAXIMUMBRIGHTNESS - MINIMUMBRIGHTNESS) / 2) ? sunImage
					: moonImage;
			String dayTimeString = (dayTimeImage == sunImage) ? "Daytime"
					: "Nighttime";
			...
			if (airspace.getScore().getCurrentMultiplier() != 1) {
				g.drawString(
						"x"
								+ String.valueOf(airspace.getScore()
			...
			if (airspace.getScore().getProgressionTowardsNextMultiplier() != 0) {
				...
				if (this.currentCoord != this.targetCoord
					...
					g.fillRect(0, this.currentCoord, 11,
							(600 - this.currentCoord));
					g.setColor(Color.white);
					this.currentCoord--;
				}
					...
					this.currentCoord++;
					if (this.currentCoord == this.targetCoord) {
						airspace.getScore().setNegMult(false);
					}
				}
			...
			if (airspace.getScore().getAchievements().getAchievementGained()) {
				...
				g.drawString(
						...
						stateContainer.Game.MAXIMUMWIDTH
								- font.getWidth(airspace.getScore()
										.scoreAchievement()) - 100, 30);
				g.drawString(
						achievementMessage,
						stateContainer.Game.MAXIMUMWIDTH - 10
								- font.getWidth(achievementMessage), 40);

				/*
				 * Decrease the synchroniser to make the text disappear after a
				...
				if (synch == 0) {
					airspace.getScore().getAchievements()
							.setAchievementGained(false);
					synch = 180;
				}
			}
		}
	...
	public void update(GameContainer gc, StateBasedGame sbg, int delta)
		...
		// Cecks if the game has been retried and if it has resets the
		// airspace
		...
		if (settingDifficulty) {
			...
			/*
			 * Check if user deserves an achievement for staying long enough in
			 * the game
			 */
			...
			if (secs >= 60) {
				secs -= 60;
				mins += 1;
				// {!} should do +60 score every
				// minute(possibly)
				// - after 3 minutes adds on 2 less points every
				// time?
			...
			if (input.isMousePressed(Input.MOUSE_LEFT_BUTTON)) {
				...
				if ((posX > 1120 && posX < 1165) && (posY > 565 && posY < 600)) {
					((Game) sbg).setAchievements(airspace.getScore()
							.getAchievements());
					((Game) sbg).setCurrentScore(airspace.getScore()
							.getCurrentScore());
					sbg.enterState(stateContainer.Game.PAUSESTATE);
				}

				/* If user pressed the music button */
				else if ((posX > 1160 && posX < 1200)
						&& (posY > 565 && posY < 600)) {
			...
			/*
			 * if (input.isKeyPressed(Input.KEY_X)){ Tornado tornado = new
			 * Tornado(airspace); tornado.init(gc); tornado.attack();
			 * airspace.getEventController().getListOfTornados ().add(tornado);
			 * }
			 */

			if (!gameplayMusic.playing() && (!musicPaused)) {
				// Loops gameplay music based on random number
				// created in init (what?)
