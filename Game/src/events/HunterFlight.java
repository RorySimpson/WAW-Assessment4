...
public class HunterFlight {
	...
	/**
	 * generateVictim: Selects a flight in the airspace for the hunter flight to
	 * chase.
	 */
	...
	public void generateVictim(Airspace airspace) {
		...
		// Otherwise loop through airspace and choose victim.
			...
			for (int i = 0; i < airspace.getListOfFlightsInAirspace().size(); i++) {
				if (!airspace.getListOfFlightsInAirspace().get(i)
						.getFlightPlan().getEntryPoint().isRunway()) {
					hasVictim = true;
	...
	/**
	 * generateX: Selects whether the flight will enter from the top left hand
	 * corner or the top right hand corner.
	 */

	public int generateX() {
		// If random number is less than 0.5, then spawn on left hand
		// side
		if (Math.random() < 0.5) {
			return 0;
		}
	...
	/**
	 * drawHunterFlight: Draws the graphical representation of the hunter flight
	 * in the airspace.
	 */
