...
public class EventController {
	...
	public EventController(Airspace airspace) {
		...
		this.nextHunterFlightTime = 3600; // One minute initial wait
											// until spawn
		this.nextTornadoTime = 1800; // 30 second initial wait until
										// spawn
	...
	/**
	 * newRadioMalfunctionTime: Generates a new point in time at which a radio
	...
	public int newRadioMalfunctionTime() {
		...
		// Radio Malfunction occurs at a time between 4 minutes and 6
		// minutes
	...
	/**
	 * newHunterFlightTime: Generates a new point in time at which a hunter
	 * flight will spawn.
	 * 
	 */
	...
	/**
	 * newHunterTornadoTime: Generates a new point in time at which a tornado
	 * will spawn.
	 * 
	 */
	...
	/**
	 * updateRadioMalfunctionEvent: Updates the state of the radio malfunction
	 * event.
	 */
	...
	public void updateRadioMalfunctionEvent() {
		...
		if (this.timeUntilRadioMalfunction == 0) {
			...
			if (this.airspace.getListOfFlights().size() > 0) {
				...
				if (this.airspace.getControls().getSelectedFlight() == this.airspace
						.getListOfFlights().get(indexOfVictimFlight)) {
					this.airspace.getControls().setSelectedFlight(null);
				}
				if (!this.airspace.getListOfFlights().get(indexOfVictimFlight)
						.isTakingOff()
						&& !this.airspace.getListOfFlights()
								.get(indexOfVictimFlight).isWaitingToTakeOff()) {
					this.airspace.getListOfFlights().get(indexOfVictimFlight)
							.setSelected(false);
					this.airspace.getListOfFlights().get(indexOfVictimFlight)
	...
	/**
	 * init: Initialises all the resources required for the event controller
	 * class, and any other classes that are rendered within it
	 * 
	 * @param gc
	 *            GameContainer
