...
public class FlightMenu implements MouseListener {
	/* Images */
	private static Image sliderBase, sliderRingBase, sliderIndicator,
			sliderIndicatorSelect, button, buttonSelect;

	/* Data needed for graphics */
	private int altSize = 100, speedSize = 100, headingSize = 120, // slider
																	// lengths
			sliderWidth = 12, indicatorSize = 20, // track and indicator
													// sizes
			buttonWidth = 55, buttonHeight = 25, // buttonSizes
			spacingSize = 6; // spacing between components

	/* Scaled instance copies */
	private Image altBase, speedBase, headingBase, aIndicator,
			aIndicatorSelect, aButton, aButtonSelect;
	/* Font instances */
	...
	private Point2D.Float altPos, speedPos, headingPos, cmdPos, abortPos,
			altMarkerPos, speedMarkerPos, altIndicatorPos, speedIndicatorPos,
			headingIndicatorPos;
	...
	/* Active subcomponent */
	/*
	 * No mode selected by default (mode refers to the radial menu modes such as
	 * speed changing or heading changing
	 */
	private int mode = NONE;
	/* Components needed for the radial controller */
	private static final int NONE = 0, ALT = 1, SPEED = 2, HEADING = 3,
			CMD = 4, ABORT = 5;
	/* Normalised indicator positions, range 0-1 | heading in radians */
	private double altIndicator, speedIndicator, headingIndicator;
	...
	public void init() throws SlickException {
		if (sliderBase == null)
			sliderBase = new Image(
					"res/graphics/FlightMenu/rectangle_slider.png");
		if (sliderRingBase == null)
			sliderRingBase = new Image(
					"res/graphics/FlightMenu/circle_slider.png");
		if (sliderIndicator == null)
			sliderIndicator = new Image(
					"res/graphics/FlightMenu/slider_element.png");
		if (button == null)
	...
	public void render(Graphics g, GameContainer gc) throws SlickException {
		...
		if (flight != null) {
			...
			if (headingBase == null)
				headingBase = sliderRingBase.getScaledCopy(headingSize,
						headingSize);
			if (aIndicator == null)
				aIndicator = sliderIndicator.getScaledCopy(indicatorSize,
						indicatorSize);
			if (aIndicatorSelect == null)
				aIndicatorSelect = sliderIndicatorSelect.getScaledCopy(
						indicatorSize, indicatorSize);
			if (aButton == null)
				aButton = button.getScaledCopy(buttonWidth, buttonHeight);
			if (aButtonSelect == null)
				aButtonSelect = buttonSelect.getScaledCopy(buttonWidth,
						buttonHeight);
			...
			if (flight.isCommandable()) {
				...
				drawImage(altBase, new Point2D.Float(altPos.x + sliderWidth,
						altPos.y));
				// account for image mispositioning after
				// rotation
				if (altMarkerPos.y == constrain(altMarkerPos.y, altPos.y,
						altPos.y + altSize))
					drawLine(g, altMarkerPos.x, altMarkerPos.y, altMarkerPos.x
							+ sliderWidth, altMarkerPos.y);
				drawString(String.valueOf(flight.getMinAltitude()), labelFont,
						labelColor, altPos.x, altPos.y + altSize); // centred on
																	// bottom
																	// left
																	// edge of
																	// slider
				drawString(String.valueOf(flight.getMaxAltitude()), labelFont,
						labelColor, altPos.x, altPos.y); // centred on top left
															// edge
															// of slider
				if (ALT == mode)
				...
				if (speedMarkerPos.x == constrain(speedMarkerPos.x, speedPos.x,
						speedPos.x + speedSize))
					drawLine(g, speedMarkerPos.x, speedMarkerPos.y,
							speedMarkerPos.x, speedMarkerPos.y + sliderWidth);
				drawString(String.valueOf(flight.getMinVelocity()), labelFont,
						labelColor, speedPos.x, speedPos.y + sliderWidth); // centred
																			// on
																			// bottom
																			// left
																			// edge
																			// of
																			// slider
				drawString(String.valueOf(flight.getMaxVelocity()), labelFont,
						labelColor, speedPos.x + speedSize, speedPos.y
								+ sliderWidth); // centred on
												// bottom right
												// edge of
												// slider
				if (SPEED == mode)
			...
			if (flight.getFlightPlan().getPointByIndex(0) == flight
					.getFlightPlan().getExitPoint()
					&& flight.getFlightPlan().getExitPoint().isRunway()
					&& this.airspace
							.getAirportRight()
							.getLandingApproachArea()
							.contains((float) flight.getX(),
									(float) flight.getY())
					&& flight.getCurrentHeading() >= 45
					&& flight.getCurrentHeading() <= 135
					&& flight.getCurrentAltitude() <= 2000) {
				if (CMD == mode)
				...
				{ // draw button text
					...
					cmdString = "Land";
					drawString(cmdString, buttonFont, buttonColor, cmdPos.x
							+ (buttonWidth / 2.0f), cmdPos.y
							+ (buttonHeight / 2.0f));
				}
			}
			...
			if (flight.getFlightPlan().getPointByIndex(0) == flight
					...
					&& this.airspace
							...
							.contains((float) flight.getX(),
									(float) flight.getY())
					&& flight.getCurrentHeading() >= 225
					&& flight.getCurrentHeading() <= 315
					&& flight.getCurrentAltitude() <= 2000) {
				if (CMD == mode)
					drawImage(aButtonSelect, cmdPos);
				else
			...
			if (flight.getAltitude() == 0 && !flight.isTakingOff()) {
				...
				if (flight.getFlightPlan().getEntryPoint() == airspace
						.getAirportLeft().getEndOfRunway()) {

					/*
					 * Set the position for the "Take off" button
					 */
					cmdPos.setLocation(flight.getX() + 50, -spacingSize / 2.0
							- buttonHeight);
					...
					cmdPos.setLocation(altPos.x - spacingSize - buttonWidth,
							-spacingSize / 2.0 - buttonHeight);
				}
				/* If it's at the right airport */
				else if (flight.getFlightPlan().getEntryPoint() == airspace
	 ...
	 *            y coordinate
	 */
	private void drawString(String str, TrueTypeFont font, Color color,
			float x, float y) {
		font.drawString(
				(float) (x - (font.getWidth(str) / 2.0) + flight.getX()),
				(float) (y - (font.getHeight() / 2.0) + flight.getY()), str,
				color);
	}
	...
	private void position() {
		...
		cmdPos.setLocation(altPos.x - spacingSize - buttonWidth, -spacingSize
				/ 2.0 - buttonHeight);
		abortPos.setLocation(altPos.x - spacingSize - buttonWidth,
				spacingSize / 2.0);
	...
	private void setMarkerPos() {
		if (flight != null) {
			altMarkerPos.setLocation(
					...
					altPos.y
							+ multScale(
									normalScale(flight.getAltitude(),
											flight.getMinAltitude(),
											flight.getMaxAltitude()), altSize,
									0));
			...
			// rescale from velocity to pixels
					speedPos.x
							+ multScale(
									normalScale(flight.getVelocity(),
											flight.getMinVelocity(),
											flight.getMaxVelocity()), 0,
									speedSize), speedPos.y);
		}
	}
	...
	private void setIndicatorPos() {
		if (flight != null) {
			...
			hr = headingSize / 2.0, sr = sliderWidth / 2.0, ir = indicatorSize / 2.0;

			altIndicatorPos.setLocation(altPos.x + sr - ir, altPos.y
					+ multScale(altIndicator, altSize, 0) - ir);
			...
			headingIndicatorPos.setLocation(
					(hr - sr) * Math.sin(headingIndicator) - ir, (hr - sr)
							* -Math.cos(headingIndicator) - ir);
		}
	}

	private double normalScale(double pos, double min, double max) {
		// return the position of pos on the scale min-max, normalised
		// to 0-1
		return (pos - min) / (max - min);
	}

	private double multScale(double normPos, double min, double max) {
		// return the actual position on the scale min-max, of
		// normalised position normPos
		return min + (normPos * (max - min));
	}

	private int constrain(int val, int min, int max) {
		// return value no more than min, no more than max, otherwise
		// val
	...
	public void mouseDragged(int oldx, int oldy, int newx, int newy) {
		...
		case ALT: {
			...
			y = constrain(y, 0, altSize); // cap y to within bounds
											// of scale
			altIndicator = normalScale(y, altSize, 0); // invert
														// scale
														// direction
			setIndicatorPos();
			break;
		...
		case SPEED: {
			...
			x = constrain(x, 0, speedSize); // cap x to within
											// bounds of scale
		...
		case HEADING: {
			...
			x = newx - flight.getX(), y = newy - flight.getY();
			headingIndicator = Math.atan2(y, x) + PI / 2; // correct
															// for
															// polar
															// coordinates
			headingIndicator = (headingIndicator < 0) ? headingIndicator
					+ (2 * PI) : headingIndicator;
			setIndicatorPos();
			break;
		}
	...
	public void setFlight(Flight flight) {
		...
		this.flight = flight;
		if (flight != null) {
			altIndicator = normalScale(flight.getTargetAltitude(),
					flight.getMinAltitude(), flight.getMaxAltitude());
			speedIndicator = normalScale(flight.getTargetVelocity(),
					flight.getMinVelocity(), flight.getMaxVelocity());
