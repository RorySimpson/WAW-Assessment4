...
public class WaypointCoop extends Waypoint {
	...
	public void render(Graphics g, AirspaceCoop airspace) throws SlickException {
		...
		if (controls.getSelectedFlight1() != null) {
			if (controls.getSelectedFlight1().getFlightPlan().getCurrentRoute()
					.indexOf(this) == 0) {
		...
		if (controls.getSelectedFlight2() != null) {
			if (controls.getSelectedFlight2().getFlightPlan().getCurrentRoute()
					.indexOf(this) == 0) {
				image = getNextWaypointImage();
		...
		// If both players selected flight's next waypoint is the same,
		// surround it in a neutral colour.
		if (controls.getSelectedFlight2() != null
				&& controls.getSelectedFlight1() != null) {
			if (controls.getSelectedFlight2().getFlightPlan().getCurrentRoute()
					.indexOf(this) == 0
					&& controls.getSelectedFlight1().getFlightPlan()
