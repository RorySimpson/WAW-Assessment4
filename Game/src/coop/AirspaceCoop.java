...
public class AirspaceCoop extends Airspace {
	...
	/**
	 * resetAirspace: Resets variables for the airspace when the game is
	 * restarted.
	 */
	@Override
	...
	public boolean newFlight(GameContainer gc) throws SlickException {
		...
		if (this.getListOfFlightsInAirspace().size() < this
			...
			if ((this.getNumberOfGameLoopsSinceLastFlightAdded() >= 850 || this
					.getListOfFlightsInAirspace().isEmpty())) {
				...
				if (this.getListOfFlightsInAirspace().isEmpty()) {
					...
					checkNumber = rand.nextInt(this
							.getRandomNumberForFlightGeneration());
				}
				...
				if (checkNumber == 1) {
					...
					// If airport left is entrypoint, line
					// it up on the runway
					if (tempFlight.getFlightPlan().getEntryPoint() == this
							.getAirportLeft().getEndOfRunway()) {
						heading = this.getAirportLeft().getRunwayHeading();
					}
					// If airport right is entrypoint, line
					// it up on the runway
					else if (tempFlight.getFlightPlan().getEntryPoint() == this
							.getAirportRight().getEndOfRunway()) {

						heading = this.getAirportRight().getRunwayHeading();
					}
					// Otherwise calculate heading to first
					// waypoint
					else {
	...
	/**
	 * newCoopFlight: Allocates a flight to the correct player and configures
	 * the flight appropriately.
	 */
	public void newCoopFlight(GameContainer gc) throws SlickException {
		...
		if (this.newFlight(gc)) {
			FlightCoop addedFlight = (FlightCoop) this
	...
	public void update(GameContainer gc) throws SlickException {
		...
		// If the time is appropriate, increase the difficulty of the
		// game
	...
	public void setListOfFlightsPlayer1(
			ArrayList<FlightCoop> listOfFlightsPlayer1) {
	...
	public void setListOfFlightsPlayer2(
			ArrayList<FlightCoop> listOfFlightsPlayer2) {
