...
public class PlayCompetitiveState extends PlayState {
	...
	public void render(GameContainer gc, StateBasedGame sbg, Graphics g)
		...
		g.drawString(
				"Player 1:    " + Integer.toString(airspace.getPlayer1Score()),
				440, 573);
		g.setColor(Color.red);
		g.drawString(
				"Player 2:    " + Integer.toString(airspace.getPlayer2Score()),
				650, 573);
		g.setColor(Color.white);
		...
		// Render Time in Multiplier Bar. Completely filled when
		// gameover.
		g.setColor(Color.cyan);
		g.fillRect(0, 600 - 600 * (secondsOccurred / 300), 11,
				600 * (secondsOccurred / 300));
		g.setColor(Color.white);
		...
		for (CrashCompetitive crash : airspace.getSeparationRules()
				.getListOfActiveCrashes()) {
			explosion.draw((float) crash.getPointOfCrash().getX() - 50,
					(float) crash.getPointOfCrash().getY() - 90);
		}
	...
	public void update(GameContainer gc, StateBasedGame sbg, int delta)
		...
		if (!gameplayMusic.playing() && (!musicPaused)) {
			// Loops gameplay music based on random number created
			// in init
