...
public class AirspaceCompetitive extends Airspace {
	...
	private int numberOfGameLoopsSinceLastPlayer1FlightAdded,
			numberOfGameLoopsSinceLastPlayer2FlightAdded;
	...
	public AirspaceCompetitive() {
		...
		numberOfGameLoopsSinceLastPlayer1FlightAdded = 60 * 5; // These
																// are
																// set
																// to
																// this
																...
																// flights
																// spawn,
		numberOfGameLoopsSinceLastPlayer2FlightAdded = 60 * 5; // on the
																// game
																// being
																// started.
	...
	// Overriden in order to create an instance of
	// SeparationRulesCompetitive
	@Override
	public void createAndSetSeparationRules() {
		this.separationRules = new SeparationRulesCompetitive(
				difficultyValueOfGame);
	...
	public void checkWhatPlayerNeedsFlight() {
		if ((this.listOfFlightsPlayer1.size() <= this.listOfFlightsPlayer2
				.size())) {
	...
	/**
	 * newCompetitiveFlight: If a flight was added to the airspace assign the
	 * flight to one of the two players.
	 */
	...
	public void newCompetitiveFlight(GameContainer gc) throws SlickException {
		...
		if (this.newFlight(gc)) {
			int heading;
			FlightCompetitive addedFlight = (FlightCompetitive) this
					.getListOfFlightsInAirspace().get(
							this.getListOfFlightsInAirspace().size() - 1);

			// If next flight should be given to player 1 then set
			// up flight so it behaves accordingly.
			...
			// If next flight should be given to player 2 then set
			// up flight so it behaves accordingly.
			else {
			...
			// Make sure flights are assigned correct initial
			// heading.
	...
	/**
	 * newFlight: Checks whether a flight should be added into the airspace and
	 * adds a flight if it's the appropriate time.
	 */
	...
	public boolean newFlight(GameContainer gc) throws SlickException {
		...
		if (this.getListOfFlightsInAirspace().size() < this
				.getMaximumNumberOfFlightsInAirspace()) {
			...
			if (this.getNumberOfGameLoopsSinceLastFlightAdded() >= (60 * 5)) {
				...
				if (addFlight(tempFlight)) {
					this.getListOfFlightsInAirspace()
							.get(this.getListOfFlightsInAirspace().size() - 1)
							.init(gc);
					return true;
				}
	...
	/**
	 * throwbackIntoAirspace: When a flight is about to leave the airspace it is
	 * spun 180 degrees back into the airspace to ensure it doesn't leave.
	 */

	public void throwbackIntoAirspace(Flight flight) {
		Random rand = new Random();
		// Flight is spun between 135 degrees to 225 degrees back into
		// the airspace.
	...
	public void removeSpecificFlight(int flight) {
		if (this.listOfFlightsPlayer1.contains(this
				.getListOfFlightsInAirspace().get(flight))) {
			this.listOfFlightsPlayer1.remove(this.getListOfFlightsInAirspace()
					.get(flight));
		} else if (this.listOfFlightsPlayer2.contains(this
				.getListOfFlightsInAirspace().get(flight))) {
			this.listOfFlightsPlayer2.remove(this.getListOfFlightsInAirspace()
					.get(flight));
		}
		...
		for (int i = 0; i < this.listOfFlightsInAirspace.size(); i++) {
			...
			if (this.listOfFlightsInAirspace.get(i).getFlightPlan()
				...
				if (cargo.getCurrentHolder() == this.getListOfFlights().get(i)) {
					cargo.setCurrentHolder(null);
					cargo.setLocation(cargo.generateRandomCargoLocation());
					updateScore((FlightCompetitive) this.getListOfFlights()
							.get(i));
				}
				this.removeSpecificFlight(i);
			} else if (this.checkIfFlightHasLeftAirspace(this
					.getListOfFlights().get(i))) {
	...
	public void setListOfFlightsPlayer1(
			ArrayList<FlightCompetitive> listOfFlightsPlayer1) {
	...
	public void setListOfFlightsPlayer2(
			ArrayList<FlightCompetitive> listOfFlightsPlayer2) {
